...
def main():
    ...
    def train():
            if args.method == 'clean':
                print('clean data!')
                concatdataset = ConcatDataset([train_dataset])
                train_sampler = RandomSampler(concatdataset)
            ...
            for _ in train_iterator:
                ...
                for step, batch in enumerate(epoch_iterator):
                    ...
                    loss1 = outputs[0]
                    logits = outputs[1]
                    loss = criterion(input = F.log_softmax(logits), target = label_matrix[batch[3]].to(device))
                    if args.gradient_accumulation_steps > 1:
                        loss = loss / args.gradient_accumulation_steps
                    ...
                    if (step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        model.zero_grad()
                        global_step += 1
                        epoch_iterator.set_description("iteration:%d, w=%.1f, Loss:%.3f" % (_, args.soft_label_weight, tr_loss/global_step))
                        if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                            evaluate('dev', global_step)
                            evaluate('test', global_step)
    ...
    def selftrain(self, soft = True):
            selftrain_dataset = ConcatDataset([train_dataset, unlabeled])
            ...
            for t3 in range(int(self_training_steps/len(train_dataloader)) + 1):
                ...
                for step, batch in enumerate(epoch_iterator):
                    ...
                    if args.self_training_contrastive_weight > 0:
                        contrastive_loss = contrastive_loss(input = torch.log(softmax(logits)), \
    ...
    if args.do_train:
        ...
        print("="*20)
        print(f'Which processor?: {args.task}')
        # print(get_labels())
