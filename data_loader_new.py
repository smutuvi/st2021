...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b, label, true = -1):
        self.guid = guid
        self.text_a = text_a
        self.text_b = text_b
        self.label = label
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, attention_mask, token_type_ids, label_id, input_mask, segment_ids, true = -1,
                 e1_mask = None, e2_mask = None, keys=None, valid_ids=None, label_mask=None):
        # self.input_ids = input_ids
        self.attention_mask = attention_mask
...
def convert_examples_to_features_others(examples, max_seq_len, tokenizer,
                                 cls_token='[CLS]',
                                 cls_token_segment_id=0,
                                 sep_token='[SEP]',
                                 pad_token=0,
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    if os.path.exists(cached_features_file) and args.auto_load:
        ...
        if args.task_type == 'wic':
            ...
            features = convert_examples_to_features_others(examples, args.max_seq_len, tokenizer, add_sep_token=args.add_sep_token, task = args.task_type)
